 ...
 */
public class homeworkupload2 extends HttpServlet {
    protected void service(HttpServletRequest request,  
		...
		for(int i=0;i<clist.size();i++){
			...
			HomeworkDaoImpl homeworkDaoImpl = (HomeworkDaoImpl) acc.getBean("homeworkDaoImpl");
			...
			for(int j=0;j<ah.size();j++){
					Plan p=planDaoImpl.findPlan(ah.get(j).getPid());
					if(p==null){
						continue;
					}
